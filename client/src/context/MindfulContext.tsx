...
// Create context
export const MindfulContext = createContext<MindfulContextType | undefined>(undefined);

// Provider component
export const MindfulProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Fetch quote data
  const { data: quoteData, isLoading: quoteLoading } = useQuery<Quote>({
  ...
  const { data: meditationData = [] } = useQuery<MeditationSession[]>({
  ...
  // Fetch resources
  const { data: resourcesData = [], isLoading: resourcesLoading } = useQuery<Resource[]>({
  ...
  const { data: featuredResourcesData = [] } = useQuery<Resource[]>({
  ...
  const createJournalEntry = async (entry: CreateJournalEntryInput) => {
    try {
  ...
  const createMeditationSession = async (session: CreateMeditationSessionInput) => {
    try {
  ...
  const calculateMoodTrend = (weeklyData: MoodData[]): string => {
